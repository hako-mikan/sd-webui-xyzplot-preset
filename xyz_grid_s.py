...
def draw_xyz_grid(p, xs, ys, zs, x_labels, y_labels, z_labels, cell, draw_legend, include_lone_images, include_sub_grids, first_axes_processed, second_axes_processed, margin_size):
    ...
    for i in range(z_count):
        ...
        if draw_legend:
            grid = images.draw_grid_annotations(grid, processed_result.images[start_index].size[0], processed_result.images[start_index].size[1], hor_texts, ver_texts, margin_size, divide = p.batch_size)
...
class Script(scripts.Script):
    ...
    def ui(self, is_img2img):
        ...
        xyzpath = xyzpath = os.path.join(os.getcwd(),"xyzpresets.json")
        ...
        def savexyzpreset_f(x_type, x_values, x_values_dropdown, y_type, y_values, y_values_dropdown, z_type, z_values, z_values_dropdown, name, mode):
            new_data = {"xtype": self.current_axis_options[x_type].label, "xvalues": x_values,"xvalues2": x_values_dropdown,
                                "ytype": self.current_axis_options[y_type].label, "yvalues": y_values,"yvalues2": y_values_dropdown,
                                "ztype": self.current_axis_options[z_type].label, "zvalues": z_values,"zvalues2": z_values_dropdown
                                }
            try:
                with open(xyzpath, 'r') as file:
                    data = json.load(file)
        ...
        def loadxyzpreset_f(name):
                ...
                if not preset_data:
                    return [gr.update(value = x) for x in ["Seed","",[],"Nothing","",[],"Nothing","",[]]]

                sets = ["xtype","xvalues","xvalues2","ytype","yvalues","yvalues2","ztype","zvalues","zvalues2"]

                return [gr.update(value = preset_data.get(x)) for x in sets]

        savexyzpreset_b.click(fn=savexyzpreset_f,inputs=[x_type, x_values, x_values_dropdown, y_type, y_values, y_values_dropdown, z_type, z_values, z_values_dropdown,xyzpresetname,xyzmode],outputs=[xyzpresets])
        loadxyzpreset_b.click(fn=loadxyzpreset_f,inputs=[xyzpresets],outputs=[x_type, x_values, x_values_dropdown, y_type, y_values, y_values_dropdown, z_type, z_values, z_values_dropdown])
